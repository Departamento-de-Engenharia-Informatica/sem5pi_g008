...
agenda_operation_room(or1,20241028,[(520,579,so100001),(1000,1020,sa003)]).
...
initialize:-
    ...
    write('For which room do you want to schedule the surgeries?'), read(Room),
    (retract(roomToUse(_));true), asserta(roomToUse(Room)),
...
generate:-
    ...
    %if v equal 1441 then the solution is not feasible
